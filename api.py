...
# 创建一个 FastAPI 应用实例
app = FastAPI(title="强脑科技产品知识问答 API", description="基于 RAG + MySQL + Redis 的智能假肢与产品综合知识问答接口")
...
async def handle_query(request: Request):
    ...
    {
        "query": "仿生手如何进行肌电校准？",
        "source_filter": "product",  // 可选，用于知识过滤
    ...
    if source_filter and source_filter not in valid_sources:
        raise HTTPException(
            status_code=400,
            detail=f"无效的知识类别。支持: {valid_sources}"
        )
