    ...
    def call_dashscope(self, prompt):
        ...
        try:
            ...
            completion = self.client.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": "你是强脑科技（含浙江强脑）智能假肢与康复辅具领域的产品知识助手，回答应准确、克制，涉及价格与医疗结论时提醒用户以官方渠道为准。"},  # 系统提示
...
def main():
    ...
    # 打印支持的知识类别
    print(f"支持的知识类别：{qa_system.config.VALID_SOURCES}")
    # 提示用户输入查询或退出
    ...
    try:
        while True:
            ...
            if query.lower() == "exit":
                ...
                break
            # 获取用户输入的知识过滤
            source_filter = input(f"请输入知识类别 ({'/'.join(qa_system.config.VALID_SOURCES)}) (直接回车默认不过滤): ").strip()
            if source_filter and source_filter not in qa_system.config.VALID_SOURCES:
                # 如果知识过滤无效，记录警告日志
                logger.warning(f"无效的知识类别 '{source_filter}'，将不过滤")
                # 设置为空，忽略过滤
                source_filter = None
            # 打印答案提示
            print("\n答案: ", end="", flush=True)
